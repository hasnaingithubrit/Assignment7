...
console.log(`The Result with Not Operator`, citizen != "Ahmad" && place != "chiniot" ? true : false); //true
...
console.log(smallName == largeName
    ...
    : "no small name and large name are different thing"); //because a and A are not equal
...
console.log("age" == "age" || "name" == "name" ? "Yes its true" : "No Both are false");
// in and both must be true
console.log("age" == "age" && "name" == "name" ? "Yes its true" : "No any one is false");
...
console.log(alien_color == "green" || "Green" ? "You enered 5 Points" : "Choose a Color");
...
let personAge = 2;
let personAgeResult = personAge < 2
    ? "Person is baby"
...
let favPizza = ["HomeTown", "Maliboti", "Tike", "Fajita", "FireHouse"];
...
let make_shirt1 = (size = "Large", text = `I Love TypeScript`) => {
    console.log(`The Size is ${size} and the text that should print on shirt is "${text}"`);
};
make_shirt1();
...
let describe_city = (city_name = "Karachi", country_name = "Pakistan") => {
...
let make_album = (name, album_title, numberOfTracks) => {
    let music_album = {
        name,
        album_title,
        numberOfTracks,
...
function Carmanufacture(manufacturer, model_name, ...options) {
    let carComponents = Object.assign({ manufacturer,
        model_name }, options);
